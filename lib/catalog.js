...
/** "From N$230" for a design, or the honest fallback. */
...
/* ---------- the kids range, as photographed ---------- *
 *
 * The client asked for every flip-top and sippy-cup photograph to appear in the
 * Kids Selection collection. Membership is the explicit `kids_item` field on the
 * photograph, not a substring match on its alt text: re-wording a caption must
 * not be able to silently drop a photo out of the gallery, which is exactly the
 * failure mode a text match would have.
 */
export const kidsWork = work.filter((w) => w.kids_item);
export const sippyWork = kidsWork.filter((w) => w.kids_item === 'sippy');
export const fliptopWork = kidsWork.filter((w) => w.kids_item === 'fliptop');

/** The priced catalogue item each kids photograph was printed on. */
const KIDS_ITEM_BLANK = { sippy: 'sippy-cup', fliptop: 'kids-fliptop' };
export const kidsWorkBlank = (w) => blankById(KIDS_ITEM_BLANK[w.kids_item]) || null;

/**
 * WhatsApp enquiry for one photographed kids item.
 *
 * The price comes from the blank record, so an unpriced item asks for
 * confirmation instead of quoting a number that does not exist.
 */
export function waKidsPhoto(w) {
  const b = kidsWorkBlank(w);
  const lines = ['Hi Gifted with Purpose, I would like to order a kids item:', ''];
  if (b) {
    lines.push(`Item: ${b.name}`);
    lines.push(`Price: ${b.price == null ? 'please confirm' : money(b.price)} each`);
  }
  lines.push(`Design: the one on photo ${w.id.replace('work-', '')} of your Kids Selection page`);
  lines.push('');
  lines.push('The name I would like printed is: ');
  return `https://wa.me/${brand.wa_number}?text=${encodeURIComponent(lines.join('\n'))}`;
}
 ...
 * confirmed item in the catalogue was the N$120 mug, so the schema was
...
/**
 * Cheapest published item, derived. Copy that quotes a floor price must use
 * this, never a literal, or it silently goes stale the next time a price moves.
 */
export const priceFloor = Math.min(...blanks.map((b) => b.price).filter((p) => typeof p === 'number'));
export const priceFloorLabel = `${brand.currency}${priceFloor}`;

/** Kids-only items, for the dedicated kids section. */
export const kidsBlanks = blanks.filter((b) => b.audience === 'kids');
export const adultBlanks = blanks.filter((b) => b.audience !== 'kids');

/**
 * Every design drawn for the kids range: sippy cup wraps and flip-top bottle
 * wraps, interleaved so the two kinds alternate. The Kids Selection page
 * leads with these, because that is what the client sends people there for.
 */
export const kidsDesigns = designs.filter((d) => d.group === 'sippy' || d.group === 'fliptop');
export const sippyDesigns = designs.filter((d) => d.group === 'sippy');
export const fliptopDesigns = designs.filter((d) => d.group === 'fliptop');

/** The kids item a kids design is printed on, for the WhatsApp message. */
export function kidsDesignBlank(design) {
  return kidsBlanks.find((b) => b.accepts.includes(design.group)) || null;
}
export const kidsPriceFloor = Math.min(
  ...kidsBlanks.map((b) => b.price).filter((p) => typeof p === 'number'),
);
