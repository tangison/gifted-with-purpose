...
### Disclaimers are now in one place
The full "we are not affiliated with any character or brand owner" wording appears **only** in
`/legal/terms`. It used to be repeated on four separate surfaces in four slightly different wordings.

Terms now also carries two clauses that were only implied before:
- **Custom artwork**: quoted per job, you confirm you have the right to what you send us, we may decline.
- **Ordering and payment**: no checkout, no account, no card details, nothing you type is stored.
- A route for rights holders to object, pointing at `sales@giftedwithpurpose.net`.

Product pages keep a single short line where a licensed design is actually on screen, linking to terms.
Removing it entirely would hide the notice at the exact moment it is relevant.

### Numbers now agree everywhere
You were right that pages disagreed. The homepage was calling the 27 legacy product photos "designs",
while every other page said 141. Two other counts were correct but worded ambiguously.

| Data point | Value, everywhere |
|---|---|
| Designs | 141 |
| Items | 11 (9 grown-up, 2 kids) |
| Blank shapes | 9 |
| Work photographs | 44 |
| Collections | 5 |
| Price floor | N$230 |

These are now read from the data files, so copy cannot drift from reality again. Two scripts
(`audit_datapoints.py`, `audit_copy.py`) re-check it on demand.
